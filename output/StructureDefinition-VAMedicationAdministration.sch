...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:MedicationAdministration">
      ...
      <sch:assert test="not(exists(f:reasonNotGiven) and f:notGiven/@value=false())">Reason not given is only permitted if NotGiven is true (inherited)</sch:assert>
      <sch:assert test="not(exists(f:reasonCode) and f:notGiven/@value=true())">Reason given is only permitted if NotGiven is false (inherited)</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
